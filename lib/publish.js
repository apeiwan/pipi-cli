...
// 检查配置文件是否存在
  ...
  function ossTask (success, fail) {
    ...
    const spinner_oss = ora('开始上传文件到' + chalk.bold(' oss ') + '服务器 ' + chalk.cyan.bold(JSON.stringify(ossConfig))).start()
    if (exec('scp -r ' + selectTask.dist + '/**' + '  root@47.96.239.153:/mnt/' + selectTask.bucket + ossConfig.prefix).code === 0) {
      spinner_oss.succeed(chalk.green.bold('oss 文件上传成功') + ' ' + chalk.cyan.bold('cdn地址：') + chalk.green.bold(cdnPath))
      success && success(cdnPath)
    } else {
      spinner_oss.fail('上传失败，请检查俺看日志')
      fail && fail()
    }
  }
  ...
  // git push
  function gitTagPush () {
    const spinner_oss = ora('开始推送项目 ' + chalk.cyan.bold(pkg.name) + ' 到 ' + chalk.cyan.bold(pkg.git) + ' 准备发布，版本号：' + chalk.cyan.bold(pkg.version)).start()
    ...
    if (exec(sourcePushExec).code === 0) {
      if (exec('git push origin --delete bundle').code !== undefined) {
        if (exec(bundlePushExec).code === 0) {
          spinner_oss.succeed('项目 ' + chalk.cyan.bold(pkg.name) + (selectTask.author ? '已部署到正式线，请确认结果' : chalk.green.bold(' 部署成功,请到钉钉进行提交部署申请')))
  ...
  function runExec () {
    ...
    function buildTask () {
      ...
      const spinner_oss = ora('开始编译 ' + chalk.cyan.bold(buildExecStr)).start()
      var code = exec(buildExecStr).code
      console.warn('code:::',code)
      if (code === 0) {
        spinner_oss.succeed('项目 ' + chalk.green.bold(pkg.name) + ' 编译成功，版本号：' + chalk.green.bold(pkg.version) + ' ')
        return 0
      }
      return -1
    }
