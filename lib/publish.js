...
async function run(projectPath,options){
  ...
  let optionTaskKey = ['staging','staging2','staging3','staging4','staging5','preview','production'];
  for(let i = 0;i < optionTaskKey.length;i++){
    ...
    if(projectConfig[key]){
      ...
      const name = item.name + '-' + item.server + '_' + cwd;
  ...
  function startTask () {
    ...
    selectTask.build = 'vue-cli-service build --mode ' + selectTask.env;
